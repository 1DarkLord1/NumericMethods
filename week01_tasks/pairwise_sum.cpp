...
int main() {
  ...
  // тогда как наивное суммирование ошибается на примерно -16% от true_result. Наивный метод работает в среднем
  // ~ 67 ms, тогда как метод попарного суммирования с simd работает ~ 40 ms, то есть simd дает ускорение примерно в 1.6 раз.
